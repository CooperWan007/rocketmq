...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx,
        ...
        log.info("receive SendMessage request command, {}", request);
         ...
         * 7、对于RETRY消息，
         *
         * 首先，会判断consumer是否还存在，因为consumer将消息返回给broker后，
         *  会设置一个延时时间，broker有一个定时任务在扫描到重发时间到了以后，
         *  会调用processor。所以要check一下调用的时候consumer还在不在。
         * 再次，消息重发是有次数限制的，默认是16次。
         *  这里会检查是否已经超过最大次数，
         *  超过的话会把消息的topic设置成并将topic设置成DeadQueue，这样后续处理中就会放入死信队列。
         */
        ...
        //如果是事务消息 并且broker不支持事务消息，返回异常信息
        ...
        //9、根据putResult设置response状态，更新broker统计信息，成功则回复producer，更新context上下文
